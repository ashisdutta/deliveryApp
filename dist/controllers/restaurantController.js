...
export const addRestaurant = async (req, res) => {
    ...
    const { name, description, street, city, state, zipCode, latitude, longitude, deliverySlabs } = result.data;
...
export const addCatagories = async (req, res) => {
    ...
    const { name, restaurantId } = result.data;
    try {
        ...
        return res.status(201).json({ message: " successfully created catagory",
            newCatagory });
    }
    catch (error) {
        if (error instanceof Error) {
...
export const addItem = async (req, res) => {
    ...
    const { name, description, price, imageUrl, categoryId, restaurantId, isAvailable } = result.data;
    try {
        const restaurant = await prisma.restaurant.findFirst({
            where: {
                id: restaurantId,
...
export const deleteCatagory = async (req, res) => {
    ...
    try {
        ...
        return res.status(201).json({ message: " catagory deleted successfully",
            deleteOperation
        });
    }
    catch (error) {
